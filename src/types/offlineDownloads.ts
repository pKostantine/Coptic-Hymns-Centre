...
export type OfflineDownloadEntityType =
  | 'music_track'
  | 'music_release'
  | 'music_playlist'
  | 'music_liked_songs'
...
export type OfflineSnapshotEntityType = OfflineDownloadEntityType
  | 'music_lyrics'
  | 'music_library';
...
export interface OfflineEntitySnapshot<T = unknown> {
  key: string;
...
export interface OfflineDownloadResource {
  ...
  playableKind?: PlaybackEntityKind | null;
  playableId?: string | null;
}

export interface OfflineDownloadRequest {
  packageKey: string;
  domain: OfflineDownloadDomain;
  ...
  snapshots: OfflineEntitySnapshot[];
}
