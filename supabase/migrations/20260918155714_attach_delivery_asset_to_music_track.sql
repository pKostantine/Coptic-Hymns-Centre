-- A track is created at submission time with no media: the file is still being
-- transcoded. When the delivery asset lands on the submission item, it has to
-- reach the track too, or publish_music_release refuses the release for having
-- tracks without published media.
...
begin
  ...
  -- Carry the asset, and the duration the probe measured, onto the track.
  update music.tracks track
  set media_asset_id = item.media_asset_id,
      duration_ms = coalesce(
        track.duration_ms,
        nullif(round((job.probe -> 'output' -> 'format' ->> 'duration')::numeric * 1000), 0)::bigint
      ),
      updated_at = now()
  from media.submission_items item
  left join media.media_processing_jobs job on job.id = item.media_processing_job_id
  where item.submission_id = p_submission_id
    and item.music_track_id = track.id
    and item.media_asset_id is not null
    and track.media_asset_id is distinct from item.media_asset_id;
