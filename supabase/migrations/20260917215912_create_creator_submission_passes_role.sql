-- Sends each item's role to add_media_submission_item instead of marking
-- artwork by prefixing its title and sorting it first. The client already
-- distinguishes artwork from media; track vs lesson is inferred from the
-- submission type, so the client does not need to know about it.
