-- Simplify creator-facing lyric sets to one editable set per language.
--
-- The public schema still keeps lyric_kind for backwards compatibility and
-- published-data consumers, but CHC Artists now uses the canonical
-- "original" row internally and exposes only language + optional description.

alter table music.lyric_sets
  add column if not exists description text;

alter table music.lyric_sets
  drop constraint if exists lyric_sets_description_not_blank;

alter table music.lyric_sets
  add constraint lyric_sets_description_not_blank
