...
begin
  ...
  set available_at = case
      ...
      -- Re-queuing an exhausted job has to clear its attempt counter too, or
      -- `claim_media_processing_job` (which only claims attempt_count <
      -- max_attempts) would skip it forever and the job would sit queued.
      attempt_count = case
        when media.media_processing_jobs.status in ('failed', 'cancelled') then 0
        else media.media_processing_jobs.attempt_count
      end,
      finished_at = case
        when media.media_processing_jobs.status in ('failed', 'cancelled') then null
        else media.media_processing_jobs.finished_at
      end,
      output_bucket = case
        when media.media_processing_jobs.status in ('failed', 'cancelled') then excluded.output_bucket
        else media.media_processing_jobs.output_bucket
      end,
      output_path = case
        when media.media_processing_jobs.status in ('failed', 'cancelled') then excluded.output_path
        else media.media_processing_jobs.output_path
      end,
      error_message = case
        when media.media_processing_jobs.status in ('failed', 'cancelled') then null
